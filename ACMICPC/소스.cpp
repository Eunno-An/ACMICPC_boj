...
int main() {
	...
	king_row = 8 - position_king[1];
	rock_row = 8 - position_rock[1];
	...
	for (int i = 0; i < N; i++) {
		...
		if (tempInput == "R") {//한 칸 오른쪽
			if (samePos(king_row, king_col + 1, rock_row, rockPos)) {
				if (ok(rock_row, rockPos, 0, 1)) {
					king_col++;
					rock_col++;
				}
				else {
			...
			if (samePos(king_row + 1, king_col, rock_row, rockPos)) {
				if (ok(rock_row, rockPos, 1, 0)) {
					king_row++;
					rock_row++;
				}
				else {
			...
			if (samePos(king_row - 1, king_col + 1, rock_row, rockPos)) {
				if (ok(rock_row, rockPos, -1, 1)) {
					king_row--;
					rock_row--;
					king_col++;
					rock_col++;
